...
const selectorSelector = (c) => `.${CSS.escape(c.className)}${":not(#\\0)".repeat(c.specificity ?? 0)}${c.selector ?? ""}`;
const propertyListSelector = (c) => `${c.property}:${c.value}${c.important ? "!important" : ""}`;
